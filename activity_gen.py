...
def generate_activities(activities: list[Activity]) -> None:
    try:
        ...
        input_activity: str = input('What type of activity are you looking for (e.g., indoor, outdoor, entertainment, adventure)? ').lower()
    except ValueError:
    ...
    for activity in activities:
        ...
        if activity_cost <= cost and activity_people <= people and input_activity == activity_type:
            matched_activities.append(activity)

    if matched_activities:
        for i, matched in enumerate(matched_activities, 1):
            print(f'{i}: {matched.activity}: {matched.activity_type}: {matched.cost}$ per person [{people} persons total spend: {people * matched.cost}$]')
    else:
